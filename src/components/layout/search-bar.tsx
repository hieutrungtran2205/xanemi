...
export function SearchBar() {
  ...
  return (
    <>
      {/* Mobile: icon trigger (hidden when overlay is open or on sm+) */}
      {!mobileExpanded && (
        <Button
          type="button"
          variant="ghost"
      ...
      {mobileExpanded && (
        <div className="fixed inset-x-0 top-0 z-60 border-b border-border bg-background sm:hidden">
          ...
          >
            ...
            <div className="relative flex flex-1 items-center gap-2 rounded-md border border-ring bg-surface px-3 py-1.5">
              ...
              {q && (
                <Button
                  ...
                  variant="ghost"
                  size="icon"
                  onClick={() => { setQ(''); setHighlightedIndex(-1); mobileInputRef.current?.focus() }}
      ...
      {/* Desktop: full input (hidden on mobile) */}
      <form
        ref={containerRef}
        onSubmit={handleSubmit}
        className={`relative hidden sm:flex items-center gap-2 rounded-md border bg-surface px-3 py-1.5 transition-[width,border-color] duration-200 ${
          expanded ? 'border-ring ml-auto w-72 flex-none' : 'border-border ml-auto w-52'
        }`}
      >
        <Search size={13} className="shrink-0 text-muted-foreground" />
        <input
        ...
        {q && (
          <Button
            ...
            variant="ghost"
            size="icon"
            onClick={() => { setQ(''); setHighlightedIndex(-1); inputRef.current?.focus() }}
            className="size-5 shrink-0"
            aria-label="Clear search"
          >
            <X size={12} />
